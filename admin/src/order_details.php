...
$update = mysqli_query($conn, "select * FROM (SELECT DISTINCT order_id,token FROM `order_details`) AS order_details INNER JOIN (SELECT * FROM `orders`) AS orders ON order_details.order_id = orders.id WHERE `order_id` = '$order_id' ");
$get = mysqli_fetch_array($update);

$get_order = mysqli_query($conn, "SELECT * FROM `orders` where `id` =$order_id ");
...
if (isset($_POST['submit'])) {
  ...
  $sql = mysqli_query($conn, "UPDATE `orders` SET `order_status` ='$status', `updated_at`=' $updated_at' where id =$order_id ");
// SEND MAIL TO USERS
            ...
            <div class="row">
             ...
             <div class="col-1">
              ...
              <div class="d-inline-block">
                ...
                >
                  Generate <i data-acorn-icon="print"></i>
                </button>
                   </form>
             </div>
           ...
           ?>
              ...
              <div class="card-body">
                <div class="row">
                  <div class="col-12">
                    ...
                    if (isset($_GET['order_id'])) {
                      $id = $_GET['order_id'];
                      $update = mysqli_query($conn, "SELECT Distinct * FROM `order_details` INNER JOIN `orders` ON order_details.order_id = orders.id where order_id =$order_id ");
                      while ($rows = mysqli_fetch_array($update)) {
                  ...
                  $rows_2 = mysqli_fetch_array($sql);
                    ...
                    <table class='table table-bordered mb-5'>
                        ...
                        <tbody>
                            <tr>
                                <td><img src="<?= 'front-store/uploads/' . $rows_2['image'] ?>" class="card-img rounded-md h-100 sw-30" alt="thumb" /></td>
            ...
            <!-- Activity End -->
              ...
              <div class="col-12 col-xl-5 col-xxl-3">
                ...
                <div class="card mb-5">
                    <div class="card-body mb-n5">
                        <form method="post" style="display:inline-block;" id="tform">
                  <?php if (isset($_GET['id '])) {
                    $id  = $_GET['id '];
                    $update = mysqli_query($conn, "SELECT * FROM `order_details` where order_id =$id ");
                    while ($rows = mysqli_fetch_array($update)) { 
                       ?>
       ...
       if (isset($_GET['order_id'])) {
           ...
           ?>
                     ...
                     if($order_status == 'Delivered'){
                       echo"
                        <div class='form-check form-switch mb-1'>
                           ...
                           <label class='form-check-label' name='status' for='delivered'>Delivered</label>
                        </div>
                       ";
                       }
                       else{
                           echo"
                        <div class='form-check form-switch mb-1'>
                           <input type='checkbox' class='form-check-input' id='quantitySwitch2' name='status' value='Delivered'>
                           <label class='form-check-label' name='status' for='delivered'>Delivered</label>
                        </div>
                       ";
                       }
                    ...
                    if($order_status == 'Pending'){
                        ?>
                         <div class="form-check form-switch mb-1">
                      <input type="checkbox" class="form-check-input" id="quantitySwitch1" name="status" value="Pending" checked>
                      <label class="form-check-label" name="status" for="Pending">Pending</label>
                    </div>
                        <?php
                       }
                       else{
                            ?>
                         <div class="form-check form-switch mb-1">
                      <input type="checkbox" class="form-check-input" id="quantitySwitch1" name="status" value="Pending">
                      <label class="form-check-label" name="status" for="Pending">Pending</label>
                    </div>
                        <?php
                       }
                     ...
                     if($order_status == 'Recieved'){
                        ?>
                    <div class="form-check form-switch mb-1">
                      <input type="checkbox" class="form-check-input" id="quantitySwitch1" name="status" value="Recieved" checked>
                      <label class="form-check-label" name="status" for="Recieved">Recieved</label>
                    </div>
                        <?php
                       } 
                       else{
                           ?>
                    <div class="form-check form-switch mb-1">
                      <input type="checkbox" class="form-check-input" id="quantitySwitch1" name="status" value="Recieved">
                      <label class="form-check-label" name="status" for="Recieved">Recieved</label>
                    </div>
                        <?php
                       }
                        ...
                        ?>
                    <div class="form-check form-switch mb-1">
                      <input type="checkbox" class="form-check-input" id="quantitySwitch1" name="status" value="Confirmed" checked>
                      <label class="form-check-label" name="status" for="Confirmed">Confirmed</label>
                    </div>
                        <?php
                       } 
                       else{
                           ?>
                    <div class="form-check form-switch mb-1">
                      <input type="checkbox" class="form-check-input" id="quantitySwitch1" name="status" value="Confirmed">
                      <label class="form-check-label" name="status" for="Confirmed">Confirmed</label>
                    </div>
                        <?php
                       }
