...
error_reporting(0);
$id = $_GET['id'];
$eQuery = "SELECT * FROM `payment_method`";
...
if (isset($_POST['save'])) {
 ...
 //   Banner Image UPLOAD
  ...
  if(!empty($img)){
    $sql = "UPDATE `payment_method` SET `img`='$img', `content`='$content'";
  }else{
   $sql = "UPDATE `payment_method` SET `content`='$content'";
  }
          ...
          <div class="col-auto mb-3 mb-md-0 me-auto">
            <div>
             <button type="button" class="backBtn text-muted" onClick="history.go(-1)">Back</button>
              <h1 class="mb-0 pb-0 display-4" id="title">Edit Payment Method</h1>
            </div>
          </div>
          <!-- Title End -->
