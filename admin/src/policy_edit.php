...
if (isset($_POST['save'])) {
 ...
 //   Banner Image UPLOAD
  ...
  if(!empty($img)){
    $sql = "UPDATE `policy` SET `img`='$img', `content`='$content'";
  }else{
  ...
  if ($query) {
    echo "<script>
    window.location.href='policy_edit.php';
    </script>";
  } else {
          ...
          <div class="col-auto mb-3 mb-md-0 me-auto">
            <div>
             <button type="button" class="backBtn text-muted" onClick="history.go(-1)">Back</button>
              <h1 class="mb-0 pb-0 display-4" id="title">Edit Policy</h1>
            </div>
          </div>
          <!-- Title End -->
