...
error_reporting(0);
$id = $_GET['id'];
$eQuery = "SELECT * FROM `terms_vendor`";
...
if (isset($_POST['save'])) {
 ...
 //   Banner Image UPLOAD
  ...
  if(!empty($img)){
    $sql = "UPDATE `terms_vendor` SET `img`='$img', `content`='$content'";
  }else{
   $sql = "UPDATE `terms_vendor` SET `content`='$content'";
  }
  ...
  if ($query) {
    echo "<script>
    window.location.href='terms-and-conditions-for-vendor.php';
    </script>";
  } else {
